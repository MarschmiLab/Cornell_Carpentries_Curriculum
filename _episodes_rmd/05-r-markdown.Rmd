...
```{r,comment=NA,echo=F,collapse=T}
cat('```{r data_summary, include=FALSE}\nnSamples <- sample_and_taxon %>%
  select(sample_id) %>%
  n_distinct()
...
```
The above plot shows the relationship between cell abundance and temperature for a total of `r '\x60r nSamples \x60'` samples. For this set of samples, temperature ranged from a minimum of `r '\x60r minTemp\x60'`°C
to a maximum `r '\x60r maxTemp\x60'`°C.
```
{: .code}
